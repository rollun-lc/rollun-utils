 ...
 * @package PlaceLocalInclude
 */
class simple_html_dom_node implements \Stringable
{
    ...
    public function __construct(private $dom)
    {
        $this->dom->nodes[] = $this;
    }
    ...
    public function individualText()
    {
        if (isset($this->_[HDOM_INFO_INNER])) {
            return $this->_[HDOM_INFO_INNER];
        }
    ...
    public function makeup()
    {
        ...
        foreach ($this->attr as $key => $val) {
            ...
            if ($val === true) {
                ...
                $quote = match ($this->_[HDOM_INFO_QUOTE][$i]) {
                    HDOM_QUOTE_DOUBLE => '"',
                    HDOM_QUOTE_SINGLE => '\'',
                    default => '',
                };
    ...
    public function __isset($name)
    {
        return match ($name) {
            ...
            'plaintext' => true,
            //no value attr: nowrap, checked selected...
            default => (array_key_exists($name, $this->attr)) ? true : isset($this->attr[$name]),
        };
    }
    ...
    public function convert_text($text)
    {
        ...
        if (!empty($sourceCharset) && !empty($targetCharset) && (strcasecmp($sourceCharset, $targetCharset) != 0)) {
            ...
            if ((strcasecmp($targetCharset, 'UTF-8') == 0) && (static::is_utf8($text))) {
                $converted_text = $text;
            } else {
 ...
 * @package PlaceLocalInclude
 */
class simple_html_dom implements \Stringable
{
    public $root = null;
    public $nodes = [];
    ...
    public function __construct($str = null, $lowercase = true, $forceTagsClosed = true, public $_target_charset = DEFAULT_TARGET_CHARSET, $stripRN = true, $defaultBRText = DEFAULT_BR_TEXT, $defaultSpanText = DEFAULT_SPAN_TEXT)
    {
        if ($str) {
